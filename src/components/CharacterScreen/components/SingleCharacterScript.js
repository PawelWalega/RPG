...
export default {
	...
	methods  : {
		...
		getTooltips() {
			for (let tier in this.character.talents) {
				const charClass = this.allClasses.find((cl) => cl.name === this.character.class);
		...
		toBattle() {
			store.dispatch('selectCharacter', this.character);
	...
	computed : {
		...
		attackOrMagic() {
			return this.magicPower < this.attackPower ? this.attackPower : this.magicPower;
		},
		dodgeChance() {
